 ...
 */
public class PushProjectIntoLookupTableSourceScanRule extends RelOptRule {
	public static final PushProjectIntoLookupTableSourceScanRule INSTANCE = new PushProjectIntoLookupTableSourceScanRule();

	public PushProjectIntoLookupTableSourceScanRule() {
		super(operand(
	...
	public void onMatch(RelOptRuleCall call) {
		final FlinkLogicalCalc project = call.rel(0);
		final FlinkLogicalTableSourceScan scan = call.rel(1);

		final List refFields = project.getProgram().getProjectList();
		...
		if (!supportsNestedProjection && refFields.size() == fieldNames.size()) {
			// just keep as same as the old plan
			// TODO: refactor the affected plan
			return;
		}

		List<RexNode> oldProjectsWithPK = project.getProgram().getProjectList()
			.stream().map(r ->  project.getProgram().expandLocalRef(r)).collect(Collectors.toList());
		...
		FlinkLogicalTableSourceScan newScan = new FlinkLogicalTableSourceScan(
		...
		List<RexNode> newProjects =
				NestedProjectionUtil.rewrite(oldProjectsWithPK, nestedSchema, call.builder().getRexBuilder());

		Calc newCalc = new FlinkLogicalCalc(
			project.getCluster(),
			project.getTraitSet(),
			newScan,
			RexProgram.create(
				newScan.getRowType(),
				newProjects,
				project.getProgram().getCondition(),
				project.getRowType(),
				project.getCluster().getRexBuilder())

		);

//		newProjects.stream().map(r -> r.getType())
		// rewrite new source
//		Calc newProject = project.copy(
//				project.getTraitSet(),
//				newScan,
//				new RexProgram(
//					project.getProgram().getInputRowType(),
//					newProjects,
//					project.getProgram().getProjectList(),
//					project.getProgram().getCondition(),
//					newProjects.)
//				);
